...
try {
  ...
  console.log("Firebase Error:", e.message); // <-- shows real error now
}
...
app.get('/', (req, res) => {
  res.json({ 
    ...
    version: "2.1",
    features: ["TV", "Podcast", "Radio", "Music", "Payments", "Newsletter", "Street Bulletin"]
  });
});
...
// =================== 7. STREET BULLETIN - NEW ===================
app.post('/bulletin/post', async (req, res) => {
  try {
    const { title, description, imageBase64, location, postedBy } = req.body;
    let imageUrl = "";
    ...
    const doc = await db.collection('bulletin').add({ title, description, imageUrl, location, postedBy, likes: 0, createdAt: new Date() });
    res.json({ success: true, id: doc.id, imageUrl });
  } catch(e) { res.status(500).json({ error: e.message }) }
});
...
app.listen(PORT, () => console.log(`ZimConnect v2.1 running on ${PORT}`));
